...
#include <chrono>
#include <cmath>
#include <opencv2/opencv.hpp>  // OpenCV library for camera capture
...
bool parse_args(int argc, char** argv, std::string& wts, std::string& engine, bool& is_p6, float& gd, float& gw, bool& use_camera) {
    ...
    if (std::string(argv[1]) == "-s" && (argc == 5 || argc == 7)) {
        ...
        if (argc == 4 && std::string(argv[3]) == "camera") {
            use_camera = true;
        }
    } else {
        return false;
...
int main(int argc, char** argv) {
    ...
    if (!parse_args(argc, argv, wts_name, engine_name, is_p6, gd, gw, use_camera)) {
        std::cerr << "arguments not right!" << std::endl;
        std::cerr << "./yolov5_det -s [.wts] [.engine] [n/s/m/l/x/n6/s6/m6/l6/x6 or c/c6 gd gw]  // serialize model to plan file" << std::endl;
        std::cerr << "./yolov5_det -d [.engine] [camera]  // deserialize plan file and run inference" << std::endl;
        return -1;
    }
    ...
    if (use_camera) {
        ...
        std::cerr << "arguments not right! Please specify 'camera' to use the camera." << std::endl;
        return -1;
    }
